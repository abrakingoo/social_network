  ...
  async fetchNotifications() {
    try {
      ...
      return notifications.map((notif) =>
        this.transformBackendNotification(notif),
      );
    } catch (error) {
      throw error;
    }
  ...
  transformBackendNotification(backendNotif) {
    ...
    if (backendNotif.actor) {
      actor = {
        ...
        firstName:
          backendNotif.actor.first_name || backendNotif.actor.firstName || "",
        lastName:
          backendNotif.actor.last_name || backendNotif.actor.lastName || "",
      ...
      actor = {
        ...
        firstName:
          backendNotif.actor_first_name || backendNotif.ActorFirstName || "",
        lastName:
          backendNotif.actor_last_name || backendNotif.ActorLastName || "",
        avatar: backendNotif.actor_avatar || backendNotif.ActorAvatar || null,
        nickname:
          backendNotif.actor_nickname || backendNotif.ActorNickname || "",
      };
    }
    ...
    if (backendNotif.entity_id && backendNotif.entity_id !== null) {
      if (
        typeof backendNotif.entity_id === "object" &&
        backendNotif.entity_id.String &&
        backendNotif.entity_id.Valid
      ) {
      ...
      if (
        typeof backendNotif.EntityID === "object" &&
        backendNotif.EntityID.String &&
        backendNotif.EntityID.Valid
      ) {
    ...
    return {
      ...
      data: {
        ...
        actor_id:
          (actor && actor.id) || backendNotif.ActorId || backendNotif.actor_id,
      },
    };
  }
  ...
  async respondToGroupInvitation(groupId, status) {
    try {
      ...
      if (!status || !["accepted", "declined"].includes(status)) {
        throw new Error(
          'Invalid status provided. Must be "accepted" or "declined"',
        );
      }
