...
const Friends = () => {
  ...
  const userSuggestions = getAllUsers ? getAllUsers().filter(
    (user) => user.id !== currentUser.id && !mockFriends.find((f) => f.id === user.id)
  ).slice(0, 5) : [];
          ...
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="requests" className="space-y-4">
              {mockFriends.filter(f => f.status === 'pending').length > 0 ? (
                mockFriends.filter(f => f.status === 'pending').map((friend) => (
                  <div key={friend.id} className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm">
                    <div className="flex items-center">
                      <Avatar className="h-12 w-12 mr-4">
                        <AvatarImage src={friend.avatar} />
            ...
            <TabsContent value="sent" className="space-y-4">
              {mockFriends.filter(f => f.status === 'requested').length > 0 ? (
                mockFriends.filter(f => f.status === 'requested').map((friend) => (
                  <div key={friend.id} className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm">
                    <div className="flex items-center">
                      <Avatar className="h-12 w-12 mr-4">
                        <AvatarImage src={friend.avatar} />
