...
func (q *Query) FetchLikedPost(userid string, user *model.UserData) error {
	...
	//use all the ids to fetch the post
	if err = q.FetchLikedPostById(postIDs, user); err != nil {
		return err
	}
...
func (q *Query) FetchLikedPostById(postIDs []string, user *model.UserData) error {
	if len(postIDs) == 0 {
		return nil // Early exit if no post IDs provided
	}
	...
	rows, err := q.Db.Query(query, args...)
	if err != nil {
		return fmt.Errorf("failed to fetch posts {user liked post} by IDs: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if existingPost, exists := postsMap[post.ID]; exists {
			...
			post.Comments = []model.Comment{} // Initialize empty comments
			postsMap[post.ID] = &post
			user.LikedPost = append(user.LikedPost, post)
		}
	}
...
func (q *Query) FetchLikedComments(userid string, user *model.UserData) error {
	...
	//use all the ids to fetch the post
	if err = q.GetLikedCommentPost(postIDs, user); err != nil {
		return err
	}
...
func (q *Query) GetLikedCommentPost(postIDs []string, user *model.UserData) error {
	...
	rows, err := q.Db.Query(query, args...)
	if err != nil {
		return fmt.Errorf("failed to fetch posts {user liked comments post} by IDs: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if existingPost, exists := postsMap[post.ID]; exists {
			...
			post.Comments = []model.Comment{} // Initialize empty comments
			postsMap[post.ID] = &post
			user.LikedComments = append(user.LikedComments, post)
		}
	}
